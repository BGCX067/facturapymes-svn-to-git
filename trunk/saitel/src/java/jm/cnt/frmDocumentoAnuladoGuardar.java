 ...
 * @author Jorge
 */
public class frmDocumentoAnuladoGuardar extends HttpServlet {
    ...
    */
    ...
    throws ServletException, IOException {
        ...
        try {
            ...
            if(!objDocumentoAnulado.estaDuplicado(id, tipo_comprobante, serie, secuencial, autorizacion)){
                r = "msg»Ha ocurrido un error inesperado, por favor, vuelva a intentarlo más tarde o " +
                        "contáctese con el administrador del sistema para mayor información.";
                if(id.compareTo("-1")==0){
                    if(objDocumentoAnulado.insertar(fecha_emision, tipo_comprobante, serie, secuencial, autorizacion)){
                        Auditoria auditoria = new Auditoria(this._ip, this._puerto, this._db, usuario, clave);
                        auditoria.setRegistro(request, "REGISTRO DE DOCUMENTO ANULADO "+serie+"-"+secuencial+" CON AUTORIZACION"+autorizacion);
                        auditoria.cerrar();
                    ...
                    if(objDocumentoAnulado.actualizar(id, fecha_emision, tipo_comprobante, serie, secuencial, autorizacion)){
                        Auditoria auditoria = new Auditoria(this._ip, this._puerto, this._db, usuario, clave);
                        auditoria.setRegistro(request, "ACTUALIZACION DE LA INFORMACION DE DOCUMENTO ANULADO "+serie+"-"+secuencial+" CON AUTORIZACION"+autorizacion);
                        auditoria.cerrar();

                        r = "tbl»"+objDocumentoAnulado.paginar("vta_documento_anulado", "id_documento_anulado,fecha_emision,serie,secuencial,autorizacion,txt_tipo_comprobante", WHERE, Integer.parseInt(p), 100)+"^msg»Información guardada satisfactoriamente.";
                    }
                }
            }
            out.print(r);
