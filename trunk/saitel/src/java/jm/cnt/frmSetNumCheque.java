 ...
 * @author Jorge
 */
public class frmSetNumCheque extends HttpServlet {
    ...
    */
    ...
    throws ServletException, IOException {
        ...
        try {
            ...
            String num_cheque = request.getParameter("num");
            String id_banco = request.getParameter("bc1");
            
            if(!objComprobanteEgreso.chequeDuplicado(id, num_cheque, id_banco)){
                if(objComprobanteEgreso.setNumCheque(id, num_cheque)){
                    String num_compEgr = "";
                    ResultSet compEgr = objComprobanteEgreso.getComprobanteEgreso(id);
                    try{
                        if(compEgr.next()){
                            num_compEgr = (compEgr.getString("num_comprobante")!=null) ? compEgr.getString("num_comprobante") : num_compEgr;
                        }
                    }catch(Exception e){
                        e.printStackTrace();
                    }
                    Auditoria auditoria = new Auditoria(this._ip, this._puerto, this._db, usuario, clave);
                    auditoria.setRegistro(request, "ACTUALIZACION DEL NUMERO DE CHEQUE "+num_cheque+" DEL COMPROBANTE DE EGRESO CON NUMERO: "+num_compEgr);
                    auditoria.cerrar();
                    r = "msg»Actualización del número de cheque "+num_cheque+" registrado satisfactoriamente.";
                }
            }else{
