 ...
 * @author Jorge
 */
public class frmValeCajaAnular extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try {
            ...
            String r = "msg»Ha ocurrido un error inesperado, por favor, vuelva a intentarlo más tarde o " +
                "contàctese con el administrador del sistema para mayor información.";

            String num_vale_caja = "0";
            try{
                ResultSet rsValeCaja = objValeCaja.getValeCaja(id);
                if(rsValeCaja.next()){
                    num_vale_caja = (rsValeCaja.getString("num_vale_caja")!=null) ? rsValeCaja.getString("num_vale_caja") : "0";
                    rsValeCaja.close();
                }
            }catch(Exception e){
                e.printStackTrace();
            ...
            if(objValeCaja.anular(id)){
                Auditoria auditoria = new Auditoria(this._ip, this._puerto, this._db, usuario, clave);
                auditoria.setRegistro(request, "ANULACION DEL VALE DE CAJA No. "+num_vale_caja);
                auditoria.cerrar();
                r = "err»0^vta»vta1^msg»Se ha anulado el vale de caja No."+num_vale_caja;
            }
