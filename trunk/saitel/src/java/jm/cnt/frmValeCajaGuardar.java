 ...
 * @author Jorge
 */
public class frmValeCajaGuardar extends HttpServlet {
    ...
    */
    ...
    throws ServletException, IOException {
        ...
        try {
            ...
            String num_vale_caja = request.getParameter("num_vale_caja");
            String fecha = request.getParameter("fecha");
            String custodio = request.getParameter("custodio");
            String concepto = request.getParameter("concepto");
            String importe = request.getParameter("importe");
            String la_suma_de = request.getParameter("la_suma_de");
            ...
            if(id.compareTo("-1")==0){
                id_comprobante = objValeCaja.insertar(id_sucursal, num_vale_caja, custodio, fecha, concepto, importe, la_suma_de, solicitante, id_plan_cuenta_gasto);
                if(id_comprobante != -1){
                    Auditoria auditoria = new Auditoria(this._ip, this._puerto, this._db, usuario, clave);
                    auditoria.setRegistro(request, "INGRESO DE NUEVO VALE DE CAJA CON NUMERO: "+num_vale_caja +" POR CONCEPTO: "+concepto);
                    auditoria.cerrar();
                    r = "err»0^vta»vta1"+(imprimir.compareTo("1")==0?"^fun»imprimir('pdfValeCaja?id="+id_comprobante+"');":"")+
                            "^tbl»"+objValeCaja.paginar("tbl_vale_caja", "id_vale_caja,num_vale_caja,toDateSQL(fecha),concepto,importe,solicitante", WHERE, Integer.parseInt(p), 100)
