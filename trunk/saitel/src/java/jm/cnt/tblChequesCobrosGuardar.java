 ...
 * @author Jorge
 */
public class tblChequesCobrosGuardar extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        while(request.getParameter("fe"+i)!=null){
            ...
            if(fecha_aux.compareTo("")!=0){
                ids_comprobante_pago += request.getParameter("idCo"+i) + ",";
        ...
        if(fechas_efectivo.compareTo("")!=0){
            ids_comprobante_pago = ids_comprobante_pago.substring(0, ids_comprobante_pago.length()-1);
        ...
        try {
            ...
            if(objComprobanteIngreso.efectivizarCheques(id_sucursal, ids_comprobante_pago, fechas_efectivo)){
                Auditoria auditoria = new Auditoria(this._ip, this._puerto, this._db, usuario, clave);
                auditoria.setRegistro(request, "CONFIRMACION DE EFECTIVIZACION DE LOS CHEQUES: "+num_cheques+" DE COBROS");
                auditoria.cerrar();
                r = "fun»cnt_conciliacion_seleccion()^msg»Confirmación de efectivización de cheques de cobros realizada satisfactoriamente.";
            }
            out.print(r);
        } finally {
