...
public class DocumentoBanco extends DataBase{
    ...
    public float getDocumentoBanco(String num_doc)
    {
        float saldo = 0;
        try{
            ResultSet res = this.consulta("SELECT monto FROM tbl_documento_banco_tmp where documento='"+num_doc+"'");
            if(res.next()){
    ...
    public boolean insertar(String usuario, String num_documento, String monto, float saldo)
    {
        return this.ejecutar("INSERT INTO tbl_documento_banco(usuario, num_documento, monto, saldo)"
