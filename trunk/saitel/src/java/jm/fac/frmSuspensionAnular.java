 ...
 * @author Jorge
 */
public class frmSuspensionAnular extends HttpServlet {
    ...
    */
    ...
    throws ServletException, IOException {
        ...
        try {
            ...
            String fecha_reactivacion = request.getParameter("fchR");
            String i = request.getParameter("i");
            String imp = request.getParameter("imp");

            int mes_susp = Fecha.datePart("mes", fecha_inicio);
            int mes_reac = Fecha.datePart("mes", fecha_reactivacion);
            if(mes_reac < mes_susp){
                mes_reac += 12;
            }
            int num_meses = mes_reac - mes_susp;
            ...
            boolean ok = objInstalacion.anularSuspension(id_suspension, usuario, fecha_reactivacion, num_meses);
            if(ok){
                Auditoria auditoria = new Auditoria(this._ip, this._puerto, this._db, usuario, clave);
                auditoria.setRegistro(request, "REGISTRO DE ANULACION DE SUSPENSION DEL SERVICIO DE INTERNET PARA LA INSTALACION NRO. "+id_sucursal+"-"+num_instalacion);
                auditoria.cerrar();
                //String imprimir = imp.compareTo("1")==0 ? "imprimir('pdfSuspensionAnulacion?id="+id_suspension+"');" : "";
                r = "fun»imprimir('pdfSuspensionAnulacion?id="+id_suspension+"');^msg»Información ingresada satisfactoriamente.";
            }
