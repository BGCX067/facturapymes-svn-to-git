 ...
 * @author Jorge
 */
public class getProductosSucursal extends HttpServlet {
    ...
    */
    ...
    throws ServletException, IOException {
        ...
        String ancho = request.getParameter("an");

        Producto objProducto = new Producto(this._ip, this._puerto, this._db, usuario, clave);
        try {
            ResultSet rsProductos = enter.compareTo("13")==0 ? objProducto.getProductoVenta(id_sucursal, texto) : objProducto.getProductosVenta(id_sucursal, texto);
            out.print("obj»ax^fun»"+funcion+"('"+objProducto.getJSON(rsProductos)+"', "+enter+", '"+prefijo+"', "+ancho+")");
            try{
                rsProductos.close();
            }catch(Exception e){
                e.printStackTrace();
