 ...
 * @author Jorge
 */
public class Suspensiones extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        response.setHeader("Content-disposition", "attachment; filename=suspensiones_"+Fecha.getFecha("ISO")+"_"+Fecha.getHora()+".txt;");
        ...
        try {
            ...
            try{
                ...
                ResultSet res = objInstalacion.getSuspensionesServidor();
                while(res.next()){                    
                    ...
                    //cliente = cliente.length()>13 ? cliente.substring(0, 13).trim() : cliente;
                    if(txt_sucursal.compareTo(ax_txt_sucursal)!=0){
                        txt += txt_sucursal + "\n";
                        ax_txt_sucursal = txt_sucursal;
                    }
                    txt += "add action=drop chain=forward comment=\""+cliente+"\" disabled=no src-address="+ip+"\n";
                }
                res.close();
            }catch(Exception e){
                e.printStackTrace();
