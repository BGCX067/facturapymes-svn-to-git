...
//import ec.gob.sri.RecepcionComprobantesService;
//import ec.gob.sri.RespuestaSolicitud;
 ...
 * @author Jorge
 */
public class frmFacturaCompraGuardar extends HttpServlet {
    ...
    */
    ...
    throws ServletException, IOException {
        ...
        try {
            ...
            String imprimir = request.getParameter("gr");
            ...
            //String iva_0 = request.getParameter("iva_0");
            ...
            for(int i=0; i<=tope; i++){
                if(request.getParameter("idA"+i)!=null){
                    ...
                    paramArtic += "['"+id_producto+"', '"+request.getParameter("c"+i)+"', '"+request.getParameter("pu"+i)+"', '"+subt+
                            "', '"+request.getParameter("d"+i)+"', '"+iva+"', '"+request.getParameter("t"+i)+"', '"+tipo+"'],";
                }
            }
            ...
            if(!objFacturaCompra.facturaDuplicada(id, id_proveedor, serie_factura, num_factura)){
                if(!objFacturaCompra.retencionDuplicada(id, num_serie_ret, num_retencion)){
                    ...
                    if(id.compareTo("-1")==0 && contabilizar.compareTo("true")==0){  // nuevo y contabilizar
                        ...
                        if(doc_electronico.compareTo("1")==0){
                            ok = false;
                        ...
                        if(ok){
                            id_factura_id_compDiario = objFacturaCompra.insertar(id_sucursal, serie_factura, autorizacion, num_factura, id_proveedor, fecha_compra, fecha_ven_factura,
                                    observacion, subtotal, subtotal_0, subtotal_2, subtotal_6, descuento, iva_2, propina, total_pagado, "array[" + paramArtic + "]", sustento_tributario, 
                                    num_serie_ret, autorizacion_ret, num_retencion, fecha_emision_ret, ret_ejercicio_fiscal_mes, ejercicio_fiscal, total_retenido, 
                                    "array[" + paramRet + "]::varchar[]", "array[" + paramAsiento + "]", id_rol, autorizacionXml);
                            if(id_factura_id_compDiario.compareTo("-1:-1") != 0){
                                ...
                                r = "err»0^vta»cmp^fun»inv_compraEditar("+vec[0]+");"+(imprimir.compareTo("1")==0?"imprimir('pdfComprobanteDiario?id="+vec[1]+"');":"")+
                        ...
                        if(id.compareTo("-1")!=0 && contabilizar.compareTo("true")==0){  //  no nuevo y contabilizar 
                            ...
                            if(ok){
                                id_factura_id_compDiario = objFacturaCompra.contabilizar(id, id_sucursal, serie_factura, autorizacion, num_factura, id_proveedor, fecha_compra, fecha_ven_factura,
                                        observacion, subtotal, subtotal_0, subtotal_2, subtotal_6, descuento, iva_2, propina, total_pagado, "array[" + paramArtic + "]", sustento_tributario, 
                                        num_serie_ret, autorizacion_ret, num_retencion, fecha_emision_ret, ret_ejercicio_fiscal_mes, ejercicio_fiscal, total_retenido, 
                                        "array[" + paramRet + "]::varchar[]", "array[" + paramAsiento + "]", id_rol, autorizacionXml);
                                if(id_factura_id_compDiario.compareTo("-1:-1") != 0){
                                   // String documento = objFacturaCompra.getTipoDocumento(id);
                                    objFacturaCompra.setEstadoDocumento(id, "f", estadoDocumento, claveAcceso, autorizacionXml, numAutorizacion);
                                    ...
                                    String vec[] = id_factura_id_compDiario.split(":");
                                    r = "err»0^vta»vta1^fun»inv_compraEditar("+vec[0]+");"+(imprimir.compareTo("1")==0?"abrir('pdfComprobanteDiario?id="+vec[1]+"');":"")+
                                        "^tbl»"+objFacturaCompra.paginar("vta_factura_compra_retencion", "id_factura_compra,razon_social,numero_factura,toDateSQL(fecha_compra),total_compra,txt_anulado,txt_estado_documento,mensaje", WHERE, Integer.parseInt(p), 100)+
                                        "^msg»Factura de compra registrada satisfactoriamente.";
                                }else{
                                    r = "msg»"+objFacturaCompra.getError();
                                }
                            ...
                            if(id.compareTo("-1")==0 && contabilizar.compareTo("false")==0){  // nuevo y no contabilizar 
                                String id_factura_compra = objFacturaCompra.guardar(id_sucursal, serie_factura, autorizacion, num_factura, id_proveedor, fecha_compra, fecha_ven_factura,
                                    observacion, subtotal, subtotal_0, subtotal_2, subtotal_6, descuento, iva_2, propina, total_pagado, paramArtic, num_serie_ret, autorizacion_ret, num_retencion, fecha_emision_ret,
                                ...
                                if(id_factura_compra.compareTo("-1")!=0){
                                    ...
                                    auditoria.cerrar();
                                    r = "err»0^vta»vta1^fun»inv_compraEditar("+id_factura_compra+");"
                                        + "^tbl»"+objFacturaCompra.paginar("vta_factura_compra_retencion", "id_factura_compra,razon_social,numero_factura,toDateSQL(fecha_compra),total_compra,txt_anulado,txt_estado_documento,mensaje", WHERE, Integer.parseInt(p), 100)+
                                ...
                                if(objFacturaCompra.actualizar(id, serie_factura, autorizacion, num_factura, id_proveedor, fecha_compra, fecha_ven_factura,
                                    observacion, subtotal, subtotal_0, subtotal_2, subtotal_6, descuento, iva_2, propina, total_pagado, paramArtic, num_serie_ret, autorizacion_ret, num_retencion, fecha_emision_ret,
                                    ret_ejercicio_fiscal_mes, ejercicio_fiscal, total_retenido, paramRet)){
                                    Auditoria auditoria = new Auditoria(this._ip, this._puerto, this._db, usuario, clave);
                                    auditoria.setRegistro(request, "ACTUALIZACION DE FACTURA DE COMPRA: "+serie_factura+"-"+num_factura);
                                    auditoria.cerrar();
                                    r = "tbl»"+objFacturaCompra.paginar("vta_factura_compra_retencion", "id_factura_compra,razon_social,numero_factura,toDateSQL(fecha_compra),total_compra,txt_anulado,txt_estado_documento,mensaje", WHERE, Integer.parseInt(p), 100)+
                                        "^msg»Factura de compra guardada satisfactoriamente.";
                                }else{
                                    r = "msg»"+objFacturaCompra.getError();
                                }
                ...
                r = "msg»Número de factura para el proveedor ya ha sido registrada.";
            }
