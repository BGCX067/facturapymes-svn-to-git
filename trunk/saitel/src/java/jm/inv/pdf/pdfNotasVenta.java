 ...
 * @author Jorge
 */
public class pdfNotasVenta extends HttpServlet {
    ...
    */
    ...
    throws ServletException, IOException {
        ...
        ResultSet registros = objDB.consulta("select id_nota_venta_compra,ruc,razon_social,numero_nota_venta,autorizacion,toDateSQL(fecha_compra),total_compra "
                + "from vta_nota_venta_compra as F "
                + "where F.anulado=false and fecha_compra between '"+fecha_ini+"' and '"+fecha_fin+"' "+WHERE+" order by fecha_compra,F.id_nota_venta_compra");
        String cabTabla[] = new String [] {"Nro.", "RUC", "RAZON SOCIAL", "No. FACTURA", "AUT.", "FECHA EMISION", "RET.", "TOTAL"};
        float anchoTabla[] = new float[]{25f,70f,160f,80f,60f,50f,40f,40f};

        Reporte reporte = new Reporte(false, logo, titulo, ruc, subtitulo, direccion, "", rep_pie);
        reporte.notasVenta(response, cabTabla, anchoTabla, registros, objDB);
